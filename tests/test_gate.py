...
def test_rejecting_one_item_does_not_discard_the_rest(conn, pile, run, cfg):
    ...
    assert counts == {"approved": len(run.gate.proposals) - 1, "rejected": 1, "ignored": 0}
    ...
    statuses = {
        r["status"]
        for r in fetch_all(conn, "SELECT status FROM conflict WHERE pile_id = %s", (pile,))
    }
...
def test_a_rejected_section_is_not_written(conn, pile, run):
    ...
    for proposal in run.gate.proposals:
        approved = not (
            proposal["kind"] == "section_patch" and proposal["payload"]["section_key"] == "billing"
        )
        gate_module.decide(
            conn, run.run_id, [Decision(str(proposal["id"]), approved)], decided_by="vinayak"
        )
...
def test_a_decision_is_final(conn, run):
    ...
    counts = gate_module.decide(
        conn, run.run_id, [Decision(str(proposal["id"]), False, "changed my mind")], "vinayak"
    )
...
def test_rejecting_everything_writes_nothing(conn, pile, run):
    for proposal in run.gate.proposals:
        gate_module.decide(conn, run.run_id, [Decision(str(proposal["id"]), False)], "vinayak")
    result = gate_module.commit(conn, pile, run.run_id, run.register)
...
def test_every_persisted_fact_has_a_span_in_its_own_document(conn, pile, run):
    ...
    for row in rows:
        ...
        page = fetch_one(
            conn, "SELECT text FROM page WHERE document_id = %s", (row["document_id"],)
        )
