...
from app.graph.incremental import run_incremental
...
def committed(conn, pile, cfg):
    ...
    try:
        result = run_understand(conn, FakeProvider(), cfg, pile, ACME)
    except MissingFixture as exc:
        pytest.skip(f"fixtures not recorded: {exc}")
    for proposal in result.gate.proposals:
        gate_module.decide(conn, result.run_id, [Decision(str(proposal["id"]), True)], "test")
    gate_module.commit(conn, pile, result.run_id, result.register)
    return result


@pytest.fixture
def arrival(conn, pile, cfg, committed):
    return run_incremental(conn, FakeProvider(), cfg, pile, ARRIVAL)
...
def test_the_untouched_sections_are_byte_identical(conn, pile, committed, arrival):
    ...
    before = {row["section_key"]: row["content_hash"]
...
def test_only_what_moved_is_proposed(conn, arrival):
    ...
    proposed = {p["payload"]["section_key"] for p in arrival.gate.proposals
...
def test_the_new_contradiction_is_surfaced(conn, arrival):
    ...
    fields = {c.field for c in arrival.new_conflicts}
    assert "hourly_rate" in fields
    assert "termination_notice_days" in fields
    rate = next(c for c in arrival.new_conflicts if c.field == "hourly_rate")
...
def test_identity_noise_is_not_reported_as_a_disagreement(conn, arrival):
    ...
    assert "counterparty" not in {c.field for c in arrival.new_conflicts}
...
def test_the_same_document_arriving_twice_changes_nothing(conn, pile, cfg, arrival):
    ...
    facts_before = fetch_one(conn, "SELECT count(*) AS n FROM fact WHERE pile_id = %s",
                             (pile,))["n"]
    again = run_incremental(conn, FakeProvider(), cfg, pile, ARRIVAL)
...
def test_committing_carries_unchanged_sections_forward(conn, pile, arrival):
    """Without this the new version would hold only the sections that moved and
    silently lose the rest."""
    for proposal in arrival.gate.proposals:
        gate_module.decide(conn, arrival.run_id, [Decision(str(proposal["id"]), True)], "test")
    result = gate_module.commit(conn, pile, arrival.run_id, arrival.register)
...
def test_carried_sections_keep_their_hash_and_get_no_audit_row(conn, pile, arrival):
    ...
    for proposal in arrival.gate.proposals:
        gate_module.decide(conn, arrival.run_id, [Decision(str(proposal["id"]), True)], "test")
    gate_module.commit(conn, pile, arrival.run_id, arrival.register)

    after = {row["section_key"]: row["content_hash"]
             for row in repo.sections_for_version(conn, pile, version=2)}
    for key in arrival.delta.unchanged:
        assert after[key] == before[key]

    audited = {row["section_key"] for row in repo.audit_trail(conn, pile)
               if row["run_id"] and str(row["run_id"]) == arrival.run_id}
    assert audited == set(arrival.delta.changed + arrival.delta.added)


def test_the_audit_names_the_document_that_caused_the_change(conn, pile, arrival):
    ...
    rows = [row for row in repo.audit_trail(conn, pile)
            if row["cause_document"] == "amendment_02.md"]
...
def test_nothing_commits_before_the_update_is_reviewed(conn, pile, arrival):
    ...
    versions = fetch_one(conn, "SELECT max(version) AS v FROM deliverable WHERE pile_id = %s",
                         (pile,))["v"]
