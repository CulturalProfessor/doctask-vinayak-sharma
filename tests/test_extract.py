    ...
    provider = ScriptedProvider(
        {
            "fields": {
                "counterparty": {
                    "value": "Acme Fabrication Services LLC",
                    "quote": "Acme Fabrication Services LLC",
                    "confidence": 1.0,
                },
...
def test_a_missing_required_field_is_reported_as_a_gap(cfg):
    ...
    result = extract_document(provider, cfg, "msa", "msa.md", MSA_TEXT)
    assert any(g.field_name == "counterparty" and "required" in g.reason for g in result.gaps)
...
def test_malformed_output_retries_once_with_a_stricter_instruction(cfg):
    provider = ScriptedProvider(
        ...
        extraction(counterparty=("Acme Fabrication Services LLC", "Acme Fabrication Services LLC")),
    )
    result = extract_document(provider, cfg, "msa", "msa.md", MSA_TEXT)
