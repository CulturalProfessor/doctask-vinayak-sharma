...
function Decided({ proposal }) {
  const how = { http: 'through the web interface', mcp: 'by a program',
                direct: 'directly' }[proposal.decided_via] ?? proposal.decided_via
  return (
...
function conflictBody(payload) {
  ...
  return {
    primary: (
      <>
        <p className="lede">
          {payload.members.length} document{payload.members.length === 1 ? '' : 's'} give
          a value for <b>{payload.field}</b>. They give {groups.size} different
          answers, and they do not agree on which one applies.
        </p>
        ...
        <p className="rationale">
          ...
          <span className="unresolved">
            Still open. Nothing has been decided or written.
          </span>
        </p>
      </>
    ),
    evidence: (
      <>
        <Subhead aside={`${payload.members.length} quotes`}>
          What each document says
        </Subhead>
        {payload.members.map((member, i) => <Span key={i} citation={member} />)}
      </>
    ),
  }
}
  ...
  return {
    primary: (
      <>
        ...
        {cited.length === 0 && (
          <p className="rationale">
            Nothing is quoted here, and that is right rather than missing. This
            finding is about a document that was set aside and never read, so it
            produced no quotes to show. The document itself is the evidence, and
    ...
    evidence: cited.length > 0 ? (
      <>
        ...
        {cited.map((citation, i) => <Span key={i} citation={citation} />)}
      </>
    ) : null,
  }
}
...
function sectionBody(payload) {
  return {
    ...
    primary: (
      <>
        <p className="lede">
          This is the text that will be added to the register if you approve it,
          exactly as it will appear. It is checked again at the moment of
          writing, and refused if anything about it has moved since you read it.
        </p>
...
function escalationBody(payload) {
  return {
    primary: (
      <>
        <p className="lede">
          This reading stopped and raised itself instead of filling the list with
          individual items. Past a certain number, disagreements stop being
          reviewable one at a time, and the reading as a whole is what needs
          attention.
        </p>
        <div className="values">
        ...
        <div className="col-main">
          ...
          {pending ? (
            <Choice
              verdict={verdict}
              onVerdict={onVerdict}
              yes={spec.yes} yesSub={spec.yesSub}
              no={spec.no} noSub={spec.noSub}
            />
          ) : (
  ...
  return (
    <>
      <div className="eyebrow">
        ...
        <span>{escalations.length} document{escalations.length === 1 ? '' : 's'}</span>
      </div>
      <h2>Is this the same company?</h2>
      <p className="lede">
        These documents name a company under a spelling this pile has not seen
        before. Filing one as a new company would split the record in two and
        hide any disagreement between the halves. Merging them is not something a
        close-looking name can settle. So they are held back, and nothing from
        them reaches the register until someone says which company they belong
        to.
      </p>
      ...
      <p className="rationale">
        The answer is recorded in the domain settings rather than by a click, so
        that it holds for the next document and the next reading too. Add the
        spelling to the list of known alternative names for that company in{' '}
        <code>config/domains/vendor_contracts/reconciliation.yaml</code>, then
        read the pile again. A name written there is treated as a decision a
        person made and beats anything the system works out on its own. If the
        companies really are different, nothing needs doing.
      </p>
      <p className="rationale unresolved">
        There is no button here on purpose. Nothing on this screen can settle
        this, and a control that wrote nothing would be worse than none.
      </p>
    </>
  )
}
  ...
  return (
    <div className="sheet">
      <div className="sheet-inner">
        <div className="eyebrow"><span className="kind">Sources</span></div>
        <h2>Check what the documents say</h2>
        <p className="lede">
          Looks through the documents that have been read and returns the
          passages that match, each one showing the document, page and exact
          position it came from. It does not answer the question, and nothing
          found here goes into the register by itself.
        </p>
        ...
        {index && (
          <div className="index-grid">
            ...
            <div>
              ...
              <div className="v" style={{ fontSize: 12.5 }}>wording, not meaning</div>
            </div>
          </div>
        )}
        ...
        {weak && (
          <div className="weak">
            ...
            <p>
              {hits.length > 0 && (
                <>
                  The closest passage scores {best.toFixed(2)}, which is a loose
                  overlap of words rather than a match worth relying on.{' '}
                </>
              )}
              {hits.length > 0 ? 'Either way that is' : 'That is'} weak evidence,
              not a finding. The search matches wording rather than meaning, so a
              clause that says this in other words would not be found. Nothing
              here supports the claim that these contracts do not cover it.
            </p>
          </div>
        )}
        ...
        {result && (
          <p className="disclaimer">
            These are sources, not an answer. The search matches wording rather
            than meaning, so finding nothing is weak evidence that there is
            nothing to find.
          </p>
        )}
      </div>
...
export function Watch({ watch }) {
  ...
  const unaccounted = pending.filter(
    (name) => !events.some((e) => e.filename === name),
  )
  const WORDS = {
    dispatched: 'read',
  ...
  return (
    <div className="sheet">
      <div className="sheet-inner">
        <div className="eyebrow"><span className="kind">Watched folder</span></div>
        <h2>
          {!watch.enabled
        ...
        <p className="lede">
          A document dropped into this folder is picked up and read on its own,
          and stops for your review like every other document. A file that sits
          here and never gets an answer is the thing worth noticing. A file that
          could not be read is not tried again.
        </p>
        ...
        <div className="watch-strip">
          ...
          <span>folder <b>{watch.directory}</b></span>
          <span>into pile <b>{watch.pile}</b></span>
          <span>checked every <b>{watch.interval_seconds}s</b></span>
        ...
        {events.length === 0 ? (
          ...
          <div className="rows">
            <div className="r head">
              <span>Result</span><span>File and reason</span><span>Reading</span><span>At</span>
            </div>
            {events.map((event, i) => (
...
export function Findings({ findings }) {
  ...
  return (
    <div className="sheet">
      <div className="sheet-inner">
        <div className="eyebrow"><span className="kind">Rule checks</span></div>
        <h2>
          {violations > 0
        ...
        <p className="lede">
          Every rule's answer, not only the broken ones. A rule that{' '}
          <em>cannot be answered</em> is not a rule that <em>holds</em>. A rule
          the documents could not answer is not a rule they passed, and treating
          the two the same is how a report comes to call a contract clean when
...
export function Register({ register }) {
  return (
    <div className="sheet">
      <div className="sheet-inner">
        <div className="eyebrow"><span className="kind">Register</span></div>
        <h2>The finished register</h2>
        <p className="lede">
          Every value here can be traced back to the passage in the document it
          came from, and anything that could not be established is written down
          as a gap rather than left blank. This is the version a person
          approved, not the version the system suggested.
        </p>
...
export function Audit({ audit }) {
  return (
    <div className="sheet">
      <div className="sheet-inner">
        <div className="eyebrow"><span className="kind">History</span></div>
        <h2>What changed, and because of which document</h2>
        <p className="lede">
          One row for each section that actually changed. Sections that stayed
          the same get no row, because nothing happened to them and a row would
          suggest otherwise.
        </p>
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Section</th><th>Because of</th><th>When</th>
              </tr>
            </thead>
            <tbody>
...
export function Stages({ report }) {
  ...
  return (
    <div className="sheet">
      <div className="sheet-inner">
        <div className="eyebrow"><span className="kind">Cost</span></div>
        <h2>What this reading spent, step by step</h2>
        <p className="lede">
          Added up from what the AI provider reported. The saved answers this
          demo replays came from a model priced at zero, so a demo reading costs
          nothing. The measurement is real; the amounts are not evidence of
          anything. <b>Which way it went</b> records the choice each step made,
          which is what makes a reading checkable rather than merely timed.
        </p>
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Step</th><th>Times</th><th>ms</th>
                <th>Sent</th><th>Received</th><th>Cost</th><th>Which way it went</th>
              </tr>
            </thead>
            <tbody>
 ...
 * question.
 */
export function Picker({ mode, corpora, value, onValue, localFile, onLocalFile,
                         onConfirm, onCancel, busy, limitMb }) {
  ...
  function take(file) {
    ...
    if (!READABLE.includes(ext)) {
      setRejected(`${file.name} is not a kind of document this reads. `
    ...
    <div className="scrim" onMouseDown={(e) => e.target === e.currentTarget && onCancel()}>
      <div className="dialog wide" role="dialog" aria-modal="true">
        ...
        <div className="picker">
          {!isFolder && (
            <div className="cap picker-divider">or one already here</div>
          )}
          ...
          {!isFolder && Object.entries(grouped).map(([folder, entries]) => (
            <div className="pick-group" key={folder}>
              ...
              {entries.map((file) => (
                <button
                  ...
                  title={file.note ?? ''}
                  onClick={() => { onLocalFile(null); onValue(file.path) }}
                >
