...
function applyTheme(theme) {
  ...
  try {
    localStorage.setItem("doctask-theme", theme);
  } catch {
    /* private mode */
  }
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    api
      .listPiles()
      .then((body) => setPiles(body.piles))
      .catch(() => {});
    api
      .watch()
      .then(setWatch)
      .catch(() => setWatch({ enabled: false }));
  }, []);
  ...
  useEffect(() => {
    ...
    api
      .corpora()
      .then(setCorpora)
      .catch(() => setCorpora({ folders: [], files: [] }));
  }, [dialog]);
  ...
  useEffect(() => {
    let current = true;
    if (!pileId) {
      // Resetting derived state when the selected pile clears, not a render loop.
  ...
  async function begin(kind) {
    ...
    const result = await guard(
      kind === "run"
        ? "reading the pile"
        : sending
          ? `sending and reading ${localFile.name}`
  ...
  async function submitReview() {
    ...
    if (!outcome) return;
    setNotice(
      `${outcome.approved} approved, ${outcome.rejected} rejected, ` +
  ...
  async function commit() {
    ...
    if (!done) return;
    setNotice(
      done.sections_written !== undefined
  ...
  async function abandon() {
    ...
    setEndReason("");
    setNotice(
      `Ended by ${done.abandoned_by}. ` +
        (done.left_undecided ? `${done.left_undecided} item(s) left undecided. ` : "") +
  ...
  async function runSearch() {
    ...
    const body = await guard("searching the documents", () => api.search(pileId, query.trim(), 8));
  ...
  function renderQueueItem(proposal, ordinal) {
    ...
    const mark =
      verdict === true || proposal.status === "approved"
        ? "a"
        : verdict === false || proposal.status === "rejected"
          ? "r"
          : null;
    return (
      <li
  ...
  return (
    <div className="app">
      <header className="top">
        ...
        <span className={`health ${health?.status ?? ""}`}>{health ? health.status : "…"}</span>
        <button
          className="theme"
          ...
          onClick={() => setTheme((prev) => THEMES[(THEMES.indexOf(prev) + 1) % THEMES.length])}
        >
          {THEME_ICON[theme]}
        </button>
      </header>
        ...
        <aside>
          <section>
            ...
            <div className="row">
              ...
              />
              <button onClick={createPile} disabled={!newPile.trim()}>
                Create
              </button>
            </div>
          </section>
            ...
            <>
              {runs.length > 0 && (
                <section>
                  ...
                  <ul className="runs">
                    {runs.map((r) => (
                      ...
                      >
                        ...
                        <span className={`state ${r.status}`}>{r.status.replace(/_/g, " ")}</span>
                        <span className="when">
                  ...
                  <p className="hint">
                    Work waiting for review is saved with the pile, not with this browser tab.
        ...
        <main>
          ...
          {!pileId ? (
            <Empty big="Choose a pile on the left, or create one.">
              Everything on this screen can also be done by a program, and this screen gets no
              shortcuts. It cannot approve or write anything a program could not, and it cannot skip
              the review.
            </Empty>
          ) : (
            <>
              {run && (
                <div className="runbar">
                  ...
                  {run.documents !== undefined ? (
                    <>
                      <span className="stat">
                        <b>{run.documents}</b> documents
                      </span>
                      <span className="stat">
                        <b>{run.facts}</b> facts
                      </span>
                      <span className="stat">
                        <b>{run.conflicts}</b> conflicts
                      </span>
                      <span className="stat">
                        <b>{run.gaps}</b> gaps
                      </span>
                    ...
                    <span className="stat">
                      opened again later, so the totals from the first reading are not shown
                    </span>
                  )}
                  ...
                  {run.unchanged !== undefined && (
                    <>
                      <span className="sep" />
                      <span className="delta added">
                        <b>{run.added.length}</b> added
                      </span>
                      <span className="delta changed">
                        <b>{run.changed.length}</b> changed
                      </span>
              ...
              <nav className="tabs">
                <div className="group">
                  ...
                  {DECIDE_TABS.map((t) => (
                    ...
                    >
                      {t.label}
                      {pending.length > 0 && <span className="badge urgent">{pending.length}</span>}
                    </button>
                  ))}
                </div>
                <span className="sep" />
                <div className="group">
                  ...
                  {EVIDENCE_TABS.map((t) => (
                    ...
                    >
                      ...
                      {t.id === "register" && !register && <span className="badge">none yet</span>}
                    </button>
                  ))}
                </div>
              </nav>
                ...
                {tab === "review" &&
                  (!run ? (
                    <Empty big="Nothing is open on this pile.">
                      Use <b>Read the pile</b> to go through it from the start, or{" "}
                      <b>Add a document</b> to bring one in. Neither one writes anything: both stop
                      here and wait for you.
                    </Empty>
                  ) : proposals.length === 0 && heldBack.length === 0 ? (
                    <Empty big="Nothing to review." good>
                      An empty list with an approve button on it teaches people to click without
                      reading, so there is not one. What happened: {run.note ?? "nothing changed."}
                    </Empty>
                  ) : (
                    <>
                      <div className="queue" ref={queueRef}>
                        ...
                        <ul>
                          ...
                          {sections.length > 0 && (
                            <li className="divider">Register sections · {sections.length}</li>
                          )}
                ...
                {tab === "findings" &&
                  (findings?.findings?.length ? (
                    ...
                    <Empty big="Nothing checked yet.">
                      Rule checks appear once the pile has been read and your rules have been put to
                      it.
                    </Empty>
                  ))}
                ...
                {tab === "register" &&
                  (register ? (
                    ...
                    <Empty big="No register yet.">
                      The register only exists after a person approves something. That is the point.
                    </Empty>
                  ))}
