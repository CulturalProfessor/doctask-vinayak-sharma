...
async function request(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    const detail =
      (parsed ? body?.detail : text.trim()) ?? `${response.status} ${response.statusText}`;
    throw new ApiError(
      response.status,
...
export const api = {
  ...
  listRuns: (pileId, status) =>
    request(`/piles/${pileId}/runs${status ? `?status=${encodeURIComponent(status)}` : ""}`),
  ...
  search: (pileId, query, limit = 8) =>
    request(`/piles/${pileId}/search?q=${encodeURIComponent(query)}&limit=${limit}`),
