...
_PATTERNS: list[tuple[str, str]] = [
    ...
    ("skips_comparison",
     r"\bdo\s+not\s+(compare|check|validate|verify|cross[-\s]?reference)\b"),
]
...
class ScreenVerdict:
    ...
    def statement(self, filename: str) -> str:
        ...
        return (f"{filename} contains text directing the analysing system to take an "
...
def screen_text(text: str, extra_patterns: list[str] | None = None) -> ScreenVerdict:
    ...
    for rule, regex in compiled:
        for match in regex.finditer(text or ""):
            hits.append(ScreenHit(rule, match.start(), match.end(),
                                  text[match.start():match.end()]))
    hits.sort(key=lambda h: h.char_start)
    return ScreenVerdict(suspicious=bool(hits), hits=hits)


def screen_pages(pages: list[tuple[int, str]],
                 extra_patterns: list[str] | None = None) -> dict[int, ScreenVerdict]:
